...
class StockAnalysisDemo:
    ...
    def get_stock_price(self, stock_id: str, period: str = "1mo") -> Dict:
        ...
        try:
            response = self.session.get(
                f"{BASE_URL}/stocks/{stock_id}/price",
    ...
    def get_realtime_price(self, stock_id: str) -> Dict:
        ...
        try:
            response = self.session.get(
                f"{BASE_URL}/stocks/{stock_id}/realtime"
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    print(f"   股利殖利率: {realtime_data.get('dividend_yield', 'N/A')}")
    ...
    def get_technical_analysis(self, stock_id: str, indicators: list = None) -> Dict:
        ...
        try:
            ...
            response = self.session.get(
                f"{BASE_URL}/analysis/{stock_id}/technical",
                params=params
            )
                ...
                if data.get("success"):
                    ...
                    if buy_signals:
                        ...
                        for signal in buy_signals:
                            print(f"     - {signal.get('indicator')}: {signal.get('reason')}")
    ...
    def get_fundamental_analysis(self, stock_id: str) -> Dict:
        ...
        try:
            response = self.session.get(
                f"{BASE_URL}/analysis/{stock_id}/fundamental"
            )
    ...
    def get_valuation_analysis(self, stock_id: str, models: list = None) -> Dict:
        ...
        try:
            ...
            response = self.session.get(
                f"{BASE_URL}/analysis/{stock_id}/valuation",
                params=params
            )
    ...
    def get_stock_prediction(self, stock_id: str, model: str = "ensemble", days: int = 30) -> Dict:
        """獲取股價預測"""
        try:
            response = self.session.get(
    ...
    def get_recommendation(self, stock_id: str) -> Dict:
        ...
        try:
            response = self.session.get(
                f"{BASE_URL}/recommendation/{stock_id}"
            )
    ...
    def get_portfolio_recommendation(self, risk_level: str = "medium", investment_amount: float = 1000000) -> Dict:
        """獲取投資組合建議"""
        try:
            response = self.session.get(
            ...
            if response.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    if stocks:
                        print(f"   股票配置:")
                        for stock in stocks:
                            print(f"     - {stock.get('name')} ({stock.get('stock_id')}): {stock.get('weight')*100:.1f}%")
