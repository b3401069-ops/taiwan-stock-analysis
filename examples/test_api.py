...
class APITester:
    ...
    def test_endpoint(self, name: str, method: str, endpoint: str, params: Dict = None, expected_status: int = 200) -> bool:
        """測試API端點"""
        try:
    ...
    def run_all_tests(self) -> Dict:
        ...
        self.test_endpoint("獲取股票價格歷史", "GET", "/stocks/2330.TW/price", {"period": "1mo"})
        ...
        self.test_endpoint("獲取股價預測", "GET", "/prediction/2330.TW", {"model": "ensemble", "days": 30})
        
        # 測試建議系統端點
        print("\n4. 建議系統端點測試")
        self.test_endpoint("獲取買賣建議", "GET", "/recommendation/2330.TW")
        self.test_endpoint("獲取投資組合建議", "GET", "/recommendation/portfolio", {"risk_level": "medium", "investment_amount": 1000000})
        
        # 測試AI Agent端點
        print("\n5. AI Agent端點測試")
        self.test_endpoint("AI Agent對話", "POST", "/agent/chat", {"message": "請分析台積電的投資價值", "agent_type": "openclaw"})
        self.test_endpoint("AI Agent分析報告", "GET", "/agent/analysis/2330.TW", {"agent_type": "openclaw"})
        
        # 測試國際股市連動分析
        print("\n6. 國際股市連動分析測試")
        self.test_endpoint("獲取國際股市連動分析", "GET", "/global/correlation", {"markets": "taiwan,japan,korea,usa"})
    ...
    def generate_report(self) -> Dict:
        ...
        # 顯示回應時間最慢的測試
        print(f"\n回應時間最慢的測試:")
        sorted_results = sorted(self.test_results, key=lambda x: x["response_time"], reverse=True)
...
def test_specific_stock(stock_id: str):
    ...
    # 測試基本資料
    print(f"\n1. 基本資料測試")
    tester.test_endpoint(f"獲取{stock_id}即時價格", "GET", f"/stocks/{stock_id}/realtime")
    
    # 測試分析功能
    print(f"\n2. 分析功能測試")
    tester.test_endpoint(f"獲取{stock_id}技術分析", "GET", f"/analysis/{stock_id}/technical")
    tester.test_endpoint(f"獲取{stock_id}基本面分析", "GET", f"/analysis/{stock_id}/fundamental")
    tester.test_endpoint(f"獲取{stock_id}估值分析", "GET", f"/analysis/{stock_id}/valuation")
    
    # 測試預測和建議
    print(f"\n3. 預測和建議測試")
    tester.test_endpoint(f"獲取{stock_id}股價預測", "GET", f"/prediction/{stock_id}", {"model": "ensemble", "days": 30})
    tester.test_endpoint(f"獲取{stock_id}買賣建議", "GET", f"/recommendation/{stock_id}")
    
    # 測試AI Agent
    print(f"\n4. AI Agent測試")
    tester.test_endpoint(f"AI Agent分析{stock_id}", "POST", "/agent/chat", {"message": f"請分析{stock_id}的投資價值", "agent_type": "openclaw"})
...
def test_performance(iterations: int = 10):
    ...
    for i in range(iterations):
        ...
        try:
            ...
            print(f"  迭代 {i+1}: {response_time:.3f}秒 (狀態碼: {response.status_code})")
...
def main():
    ...
    if choice == "1":
        ...
        # 儲存測試報告
        with open(f"test_report_{stock_id.replace('.', '_')}.json", "w", encoding="utf-8") as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
