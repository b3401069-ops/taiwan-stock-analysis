...
class DataFetcher:
            ...
            if self._is_taiwan_stock(stock_id):
                try:
                    ...
                    if not twse_df.empty:
                        logger.info(
                            f"從 TWSE 取得 {stock_id} {len(twse_df)} 筆價格資料"
                        )
                        # 同步寫入資料庫
                        try:
            ...
            if not financials.empty:
                for col in financials.columns:
                    ...
                    for idx in financials.index:
                        ...
                        if pd.notna(value):
                            financial_data["income_statement"][year][str(idx)] = float(
                                value
                            )
            ...
            if not balance_sheet.empty:
                for col in balance_sheet.columns:
                    ...
                    for idx in balance_sheet.index:
                        ...
                        if pd.notna(value):
                            financial_data["balance_sheet"][year][str(idx)] = float(
                                value
                            )
    ...
    async def get_market_indices(self, market: str = "taiwan") -> Dict:
        ...
        try:
            ...
            indices = {
                ...
                "japan": {"NIKKEI": "^N225", "TOPIX": "^TOPX"},  # 日經指數  # 東證指數
                "korea": {
                    "KOSPI": "^KS11",  # 韓國綜合指數
    ...
    async def get_global_market_data(self) -> Dict:
        ...
        try:
            ...
            return {"timestamp": datetime.now().isoformat(), "markets": global_data}
