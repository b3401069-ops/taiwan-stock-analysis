...
class StockData:
    ...
    def calculate_beta(self, stock_returns: pd.DataFrame, market_returns: pd.DataFrame) -> float:
        """計算Beta係數"""
        try:
            # 計算協方差
    ...
    def calculate_max_drawdown(self, price_data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # 計算回撤
            price_data['drawdown'] = (price_data['close'] - price_data['peak']) / price_data['peak']
    ...
    def calculate_macd(self, price_data: pd.DataFrame, fast_period: int = 12, slow_period: int = 26, signal_period: int = 9) -> Dict:
        """計算MACD指標"""
        try:
            # 計算快速EMA
    ...
    def calculate_kd(self, price_data: pd.DataFrame, k_period: int = 9, d_period: int = 3) -> Dict:
        """計算KD指標"""
        try:
            # 計算最近N天的最高價和最低價
    ...
    def calculate_bollinger_bands(self, price_data: pd.DataFrame, period: int = 20, std_dev: int = 2) -> Dict:
        """計算布林通道"""
        try:
            # 計算中軌線（移動平均線）
    ...
    def calculate_moving_averages(self, price_data: pd.DataFrame, periods: List[int] = [5, 10, 20, 50, 100, 200]) -> Dict:
        """計算移動平均線"""
        try:
            moving_averages = {}
