...
def create_stock_chart(df, title="股票價格走勢"):
    ...
    if "ma5" in df.columns:
        fig.add_trace(
            go.Scatter(
                x=df.index, y=df["ma5"], name="MA5", line=dict(color="orange", width=1)
            ),
            row=1,
            col=1,
        )
    if "ma20" in df.columns:
        fig.add_trace(
            go.Scatter(
                x=df.index, y=df["ma20"], name="MA20", line=dict(color="blue", width=1)
            ),
            row=1,
            col=1,
        )
...
def market_overview_page():
    """市場總覽頁面"""
    st.markdown(
        '<div class="main-header"><h1>📊 台灣股票分析工具 - 市場總覽</h1></div>',
        unsafe_allow_html=True,
    )
    ...
    if market_data and market_data.get("success"):
        ...
        with col1:
            ...
            st.write(
                f"**建議動作**: {regime_data.get('suggestion', {}).get('action', 'N/A')}"
            )

        with col2:
            st.write(
                f"**分析原因**: {regime_data.get('suggestion', {}).get('reason', 'N/A')}"
            )
...
def stock_analysis_page():
    """個股分析頁面"""
    st.markdown(
        '<div class="main-header"><h1>📈 個股分析</h1></div>', unsafe_allow_html=True
    )
    ...
    if st.button("開始分析"):
        with st.spinner("正在分析..."):
            ...
            if valuation_data and valuation_data.get("success"):
                ...
                with col3:
                    st.metric(
                        "股利殖利率", f"{valuation.get('dividend_yield', 0):.2f}%"
                    )
...
def screener_page():
    """多因子選股頁面"""
    st.markdown(
        '<div class="main-header"><h1>🔍 多因子選股</h1></div>', unsafe_allow_html=True
    )
    ...
    if st.button("開始篩選"):
        with st.spinner("正在篩選..."):
            result = call_api(
                "/screener/multi-factor", method="POST", params={"top_n": top_n}
            )
            ...
            if result and result.get("success"):
                ...
                if stocks:
                    ...
                    # 顯示詳細資料
                    st.dataframe(
                        stock_df[["rank", "stock_id", "stock_name", "composite_score"]]
                    )
                else:
                    st.warning("沒有篩選結果")
...
def industry_rotation_page():
    """產業輪動分析頁面"""
    st.markdown(
        '<div class="main-header"><h1>🏭 產業輪動分析</h1></div>',
        unsafe_allow_html=True,
    )
            ...
            if rotation_data and rotation_data.get("success"):
                ...
                if opportunities:
                    for opp in opportunities:
                        with st.expander(
                            f"{opp['from_industry']} → {opp['to_industry']}"
                        ):
...
def concept_rotation_page():
    """概念股輪動分析頁面"""
    st.markdown(
        '<div class="main-header"><h1>💡 概念股輪動分析</h1></div>',
        unsafe_allow_html=True,
    )
...
def ai_summary_page():
    """AI 選股摘要頁面"""
    st.markdown(
        '<div class="main-header"><h1>🤖 AI 選股摘要</h1></div>', unsafe_allow_html=True
    )
...
def backtest_page():
    """回測頁面"""
    st.markdown(
        '<div class="main-header"><h1>📊 回測分析</h1></div>', unsafe_allow_html=True
    )
    ...
    if strategies_data and strategies_data.get("success"):
        ...
        train_window = st.number_input(
            "訓練窗口 (交易日)", min_value=60, max_value=504, value=252
        )
        test_window = st.number_input(
            "測試窗口 (交易日)", min_value=21, max_value=126, value=63
        )
        step_size = st.number_input(
            "步進大小 (交易日)", min_value=1, max_value=63, value=21
        )
...
def portfolio_page():
    """虛擬倉位頁面"""
    st.markdown(
        '<div class="main-header"><h1>💼 虛擬倉位管理</h1></div>',
        unsafe_allow_html=True,
    )
        ...
        if st.button("買入"):
            if buy_stock:
                result = call_api(
                    "/portfolio/buy",
                    method="POST",
                    data={"stock_id": buy_stock, "shares": buy_shares},
                )
    ...
    with col2:
        ...
        sell_shares = st.number_input(
            "股數", min_value=1, value=1000, key="sell_shares"
        )
        ...
        if st.button("賣出"):
            if sell_stock:
                result = call_api(
                    "/portfolio/sell",
                    method="POST",
                    data={"stock_id": sell_stock, "shares": sell_shares},
                )
