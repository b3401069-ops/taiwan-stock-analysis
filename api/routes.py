...
from analysis.backtest import STRATEGIES, get_backtest_engine
...
async def get_stocks(
    market: Optional[str] = Query(
        None, description="股票市場（taiwan, japan, korea, usa）"
    ),
...
async def get_stock(stock_id: str, service: StockService = Depends(get_stock_service)):
    """獲取股票詳細資訊"""
    try:
...
async def get_stock_price(
    ...
    period: Optional[str] = Query(
        "1y", description="時間週期（1d, 5d, 1mo, 3mo, 6mo, 1y, 2y, 5y, 10y, ytd, max）"
    ),
    service: StockService = Depends(get_stock_service),
):
    """獲取股票價格歷史"""
...
async def get_technical_analysis(
    ...
    indicators: Optional[List[str]] = Query(
        None, description="技術指標列表（sma, ema, rsi, macd, kd, bollinger）"
    ),
...
async def get_fundamental_analysis(
    stock_id: str, service: AnalysisService = Depends(get_analysis_service)
):
    """獲取基本面分析結果"""
    try:
...
async def get_valuation_analysis(
    ...
    models: Optional[List[str]] = Query(
        None, description="估值模型列表（pe, pb, dividend_yield, ev_ebitda, fcf_yield）"
    ),
    service: AnalysisService = Depends(get_analysis_service),
):
    """獲取估值分析結果"""
...
async def get_stock_prediction(
    ...
    model: Optional[str] = Query(
        "ensemble",
        description="預測模型（arima, lstm, xgboost, transformer, ensemble）",
    ),
...
async def get_recommendation(
    stock_id: str, service: RecommendationService = Depends(get_recommendation_service)
):
    """獲取買賣建議"""
    try:
...
async def get_portfolio_recommendation(
    risk_level: Optional[str] = Query(
        "medium", description="風險等級（low, medium, high）"
    ),
    ...
    """獲取投資組合建議"""
    try:
        portfolio = await service.get_portfolio_recommendation(
...
async def get_global_correlation(
    markets: Optional[List[str]] = Query(
        None, description="市場列表（taiwan, japan, korea, usa）"
    ),
    service: AnalysisService = Depends(get_analysis_service),
):
    """獲取國際股市連動分析"""
...
async def get_industry_report(
    industry: str, service: AnalysisService = Depends(get_analysis_service)
):
    """獲取產業研究報告"""
    try:
...
async def agent_chat(
    ...
    agent_type: Optional[str] = Query(
        "openclaw", description="Agent類型（openclaw, hermes）"
    ),
    service: AnalysisService = Depends(get_analysis_service),
):
    """與AI Agent對話"""
...
async def get_ai_analysis(
    ...
    try:
        ...
        result = await analyst.analyze(
            stock_id, include_ml=include_ml, prediction_days=prediction_days
        )
        return {"success": True, "data": result}
    except Exception as e:
...
async def get_analysis_report(
    stock_id: str, include_ml: bool = Query(True, description="是否包含 ML 預測")
):
    """取得股票分析報告的文字摘要"""
    try:
        analyst = get_stock_analyst()
        result = await analyst.analyze(
            stock_id, include_ml=include_ml, prediction_days=5
        )
        return {
            "success": True,
            "stock_id": stock_id,
...
async def chat_with_analyst(message: str = Query(..., description="用戶訊息")):
    """與 AI 股票分析師對話"""
    try:
...
async def sell_stock(
    ...
    try:
        ...
        result = portfolio.sell_position(
            stock_id=stock_id, shares=shares, sell_price=sell_price, reason=reason
        )
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_factor_explanation():
    ...
    try:
        screener = get_multi_factor_screener()
        return {"success": True, "data": screener.get_factor_explanation()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        for factor_name, weight in weights.items():
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"無效的因子名稱: {factor_name}"
                )
...
async def walk_forward_validate(
    ...
    try:
        ...
        if strategy_name not in STRATEGIES:
            raise HTTPException(
                status_code=400, detail=f"無效的策略名稱: {strategy_name}"
            )
        ...
        return {"success": True, "data": validator.to_dict(result)}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def stock_factor_attribution(
    ...
    try:
        ...
        return {"success": True, "data": attribution.to_dict(result)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_attribution_explanation():
    ...
    try:
        attribution = get_factor_attribution()
        return {"success": True, "data": attribution.get_factor_explanation()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_industry_rotation_explanation():
    ...
    try:
        analyzer = get_industry_rotation_analyzer()
        return {"success": True, "data": analyzer.get_industry_explanation()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_concept_rotation_explanation():
    ...
    try:
        analyzer = get_concept_rotation_analyzer()
        return {"success": True, "data": analyzer.get_concept_explanation()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_ai_analysis_explanation():
    ...
    try:
        summary = get_ai_stock_summary()
        return {"success": True, "data": summary.get_ai_analysis_explanation()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def send_discord_notification(
    content: str = Query(..., description="訊息內容"), embeds: list[dict] = None
):
    """發送 Discord 通知"""
    try:
...
async def send_discord_stock_alert(
    ...
    try:
        ...
        return {
            "success": success,
            "message": (
                "Discord 股票警報發送成功" if success else "Discord 股票警報發送失敗"
            ),
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def send_discord_report(
    report_type: str = Query(..., description="報告類型"), report_data: dict = None
):
    """發送 Discord 報告"""
    try:
        notification = get_notification_service()
...
async def get_dividend_data(
    stock_id: str, years: int = Query(5, description="歷史年數")
):
    """取得股票的除權息資料和填息率"""
    try:
...
async def get_twse_stock_history(
    stock_id: str, months: int = Query(12, description="回溯月數")
):
    """取得個股歷史日成交資訊"""
    try:
        twse = get_twse_fetcher()
...
async def get_db_stocks():
    ...
    try:
        ...
        return {
            "success": True,
            "data": [
                {"stock_id": s.stock_id, "name": s.name, "market": s.market}
                for s in stocks
            ],
        }
    except Exception as e:
