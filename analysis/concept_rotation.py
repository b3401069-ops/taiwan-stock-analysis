...
class ConceptRotationAnalyzer:
            ...
            for concept_name, concept_info in self.concepts.items():
                try:
                    group = self._analyze_concept_group(
                        concept_name, concept_info, period
                    )
                    if group:
    ...
    def find_hot_concepts(
        self, period: str = "6mo", min_heat: float = 0.5
    ) -> List[ConceptGroup]:
        """
        尋找熱門概念股
    ...
    def _analyze_concept_group(
        self, concept_name: str, concept_info: Dict, period: str
        ...
        try:
            ...
            total_institutional_flow = sum(
                [s.institutional_flow for s in concept_stocks]
            )
    ...
    def _analyze_concept_stock(
        self, stock_id: str, concept_name: str, period: str
        ...
        try:
            ...
            # 計算動量指標
            momentum_20d = (
                df["Close"].pct_change(periods=20).iloc[-1] if len(df) >= 20 else 0
            )
            momentum_60d = (
                df["Close"].pct_change(periods=60).iloc[-1] if len(df) >= 60 else 0
            )
            ...
            # 判斷趨勢
            trend = self._determine_stock_trend(
                momentum_20d, momentum_60d, volume_ratio
            )
    ...
    def _determine_stock_trend(
        self, momentum_20d: float, momentum_60d: float, volume_ratio: float
    ) -> ConceptTrend:
        """判斷單一股票趨勢"""
        try:
            # 計算綜合分數
    ...
    def get_concept_explanation(self) -> Dict:
        ...
        return {
            ...
            "heat_levels": [
                {
                    "level": "hot",
                    "description": "熱門，市場關注度極高",
                    "action": "積極佈局",
                },
                {
                    "level": "rising",
                    "description": "上升，市場關注度增加",
                    "description": "適度佈局",
                },
                {
                    "level": "stable",
                    "description": "穩定，市場關注度平穩",
                    "action": "持有觀察",
                },
                {
                    "level": "declining",
                    "description": "下降，市場關注度減少",
                    "action": "減碼觀望",
                },
                {
                    "level": "cold",
                    "description": "冷門，市場關注度極低",
                    "action": "避免佈局",
                },
            ],
            "usage_tips": [
