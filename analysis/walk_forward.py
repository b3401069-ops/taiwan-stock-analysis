...
from analysis.backtest import BacktestEngine, BacktestResult, STRATEGIES
...
class WalkForwardValidator:
    ...
    def validate(
        ...
        """
        try:
            logger.info(f"開始 Walk-Forward 驗證: {stock_id}, 策略: {strategy.__name__}")
            ...
            while start_idx < end_idx:
                try:
                    ...
                    # 在訓練期間優化策略參數（簡化版本）
                    optimized_params = self._optimize_strategy_params(train_data, strategy)
            ...
            logger.info(f"Walk-Forward 驗證完成: {period_count} 個期間，平均報酬率 {avg_return:.2f}%")
    ...
    def _optimize_strategy_params(self, train_data: pd.DataFrame, strategy: Callable) -> Dict:
        """
        在訓練期間優化策略參數（簡化版本）
        ...
        try:
            ...
            if trades:
                ...
                if buy_prices and sell_prices:
                    win_trades = sum(1 for i in range(min(len(buy_prices), len(sell_prices))) 
