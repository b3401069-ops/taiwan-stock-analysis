...
class TechnicalAnalysis:
    ...
    async def analyze(self, stock_id: str, indicators: Optional[List[str]] = None) -> Dict:
        """執行技術分析"""
        try:
            # 獲取價格數據
            ...
            for indicator in indicators:
                if indicator == "sma":
                    ...
                    analysis_result["indicators"]["bollinger"] = self._calculate_bollinger(df)
                elif indicator == "volume":
                    analysis_result["indicators"]["volume"] = self._calculate_volume(df)
            
            # 生成交易信號
            analysis_result["signals"] = self._generate_signals(df, analysis_result["indicators"])
            
            # 生成分析摘要
            analysis_result["summary"] = self._generate_summary(analysis_result["indicators"], analysis_result["signals"])
        ...
        try:
            ...
            for period in periods:
                ...
                sma_data[f"sma_{period}"] = {
                    ...
                    "trend": "up" if sma.iloc[-1] > sma.iloc[-2] else "down" if sma.iloc[-1] < sma.iloc[-2] else "neutral"
                }
            ...
            if len(df) >= 200:
                ...
                for i in range(-10, 0):
                    if sma_50.iloc[i] > sma_200.iloc[i] and sma_50.iloc[i-1] <= sma_200.iloc[i-1]:
                        recent_cross = {"type": "golden_cross", "date": df.index[i].strftime("%Y-%m-%d")}
                        break
                    elif sma_50.iloc[i] < sma_200.iloc[i] and sma_50.iloc[i-1] >= sma_200.iloc[i-1]:
                        recent_cross = {"type": "death_cross", "date": df.index[i].strftime("%Y-%m-%d")}
                        break
        ...
        try:
            ...
            for period in periods:
                ...
                ema_data[f"ema_{period}"] = {
                    ...
                    "trend": "up" if ema.iloc[-1] > ema.iloc[-2] else "down" if ema.iloc[-1] < ema.iloc[-2] else "neutral"
                }
        ...
        try:
            ...
            rsi_data = {
                ...
                "statistics": {
                    ...
                    "current_zone": "overbought" if rsi.iloc[-1] > 70 else "oversold" if rsi.iloc[-1] < 30 else "neutral"
                }
            }
    ...
    def _calculate_macd(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # 判斷信號
            signal = "neutral"
            if latest_macd > latest_signal and macd_data['macd_line'].iloc[-2] <= macd_data['signal_line'].iloc[-2]:
                signal = "bullish"
            elif latest_macd < latest_signal and macd_data['macd_line'].iloc[-2] >= macd_data['signal_line'].iloc[-2]:
                signal = "bearish"
    ...
    def _calculate_kd(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # 判斷信號
            signal = "neutral"
            if latest_k > latest_d and kd_data['k_values'].iloc[-2] <= kd_data['d_values'].iloc[-2]:
                signal = "bullish"
            elif latest_k < latest_d and kd_data['k_values'].iloc[-2] >= kd_data['d_values'].iloc[-2]:
                signal = "bearish"
    ...
    def _generate_summary(self, indicators: Dict, signals: Dict) -> Dict:
        ...
        try:
            ...
            if signals["overall_signal"] == "buy":
                ...
                summary["risk_level"] = "low" if signals["confidence"] > 0.8 else "medium"
            elif signals["overall_signal"] == "sell":
                summary["recommendation"] = "sell"
                summary["risk_level"] = "high" if signals["confidence"] > 0.8 else "medium"
