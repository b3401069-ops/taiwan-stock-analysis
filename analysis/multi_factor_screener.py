...
class MultiFactorScreener:
    ...
    def _calculate_stock_score(self, stock_id: str) -> Optional[StockScore]:
        ...
        try:
            ...
            # 6. 法人因子
            institutional_score = self._calculate_institutional_factor(
                stock_id, stock_data
            )
    ...
    def _calculate_momentum_factor(self, stock_data: Dict) -> FactorScore:
        ...
        try:
            ...
            # 計算動量分數
            momentum_20 = (
                history["momentum_20"].iloc[-1]
                if "momentum_20" in history.columns
                else 0
            )
            momentum_60 = (
                history["momentum_60"].iloc[-1]
                if "momentum_60" in history.columns
                else 0
            )
    ...
    def _calculate_liquidity_factor(self, stock_data: Dict) -> FactorScore:
        ...
        try:
            ...
            # 計算成交量比率
            volume_ratio = (
                history["volume_ratio"].iloc[-1]
                if "volume_ratio" in history.columns
                else 1.0
            )
    ...
    def _calculate_institutional_factor(
        self, stock_id: str, stock_data: Dict
    ) -> FactorScore:
        """計算法人因子分數"""
        try:
            ...
            try:
                institutional_data = self.finmind_fetcher.get_institutional_investors(
                    clean_id
                )
                if (
                    institutional_data.get("success")
                    and not institutional_data["data"].empty
                ):
