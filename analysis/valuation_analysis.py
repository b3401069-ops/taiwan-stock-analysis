...
class ValuationAnalysis:
    ...
    async def analyze(self, stock_id: str, models: Optional[List[str]] = None) -> Dict:
        ...
        try:
            ...
            for model in models:
                if model == "pe":
                    analysis_result["valuation_models"]["pe"] = (
                        self._calculate_pe_valuation(financial_data, realtime_data)
                    )
                elif model == "pb":
                    analysis_result["valuation_models"]["pb"] = (
                        self._calculate_pb_valuation(financial_data, realtime_data)
                    )
                elif model == "dividend_yield":
                    analysis_result["valuation_models"]["dividend_yield"] = (
                        self._calculate_dividend_yield_valuation(
                            financial_data, realtime_data
                        )
                    )
                elif model == "ev_ebitda":
                    analysis_result["valuation_models"]["ev_ebitda"] = (
                        self._calculate_ev_ebitda_valuation(
                            financial_data, realtime_data
                        )
                    )
                elif model == "fcf_yield":
                    analysis_result["valuation_models"]["fcf_yield"] = (
                        self._calculate_fcf_yield_valuation(
                            financial_data, realtime_data
                        )
                    )

            # 計算綜合估值
            analysis_result["composite_valuation"] = (
                self._calculate_composite_valuation(analysis_result["valuation_models"])
            )
    ...
    def _calculate_pe_valuation(
        ...
        try:
            ...
            historical_pe = {"average": 15, "min": 10, "max": 25, "current": pe_ratio}
    ...
    def _calculate_dividend_yield_valuation(
        self, financial_data: Dict, realtime_data: Dict
    ) -> Dict:
        """計算股利殖利率估值"""
        try:
    ...
    def _calculate_fcf_yield_valuation(
        self, financial_data: Dict, realtime_data: Dict
    ) -> Dict:
        """計算自由現金流收益率估值"""
        try:
