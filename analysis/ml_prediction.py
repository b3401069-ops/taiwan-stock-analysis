...
class MLPrediction:
    ...
    async def predict(
        ...
        try:
            ...
            # 根據選擇的模型執行預測
            if model == "arima":
                prediction_result["predictions"]["arima"] = self._arima_predict(
                    df, days
                )
            elif model == "lstm":
                prediction_result["predictions"]["lstm"] = self._lstm_predict(df, days)
            elif model == "xgboost":
                prediction_result["predictions"]["xgboost"] = self._xgboost_predict(
                    df, days
                )
            elif model == "transformer":
                prediction_result["predictions"]["transformer"] = (
                    self._transformer_predict(df, days)
                )
            elif model == "ensemble":
                # 使用集成學習
            ...
            # 計算模型性能
            prediction_result["model_performance"] = self._evaluate_model_performance(
                split_data
            )

            # 生成摘要
            prediction_result["summary"] = self._generate_prediction_summary(
                prediction_result
            )
    ...
    def _trend_extrapolate(self, df: pd.DataFrame, days: int, model_name: str) -> Dict:
        ...
        for i in range(days):
            ...
            pred_date = (pd.Timestamp.now() + pd.Timedelta(days=i + 1)).strftime(
                "%Y-%m-%d"
            )
            predictions.append(
                {
        ...
        return {
            ...
            "expected_return": (
                (final_price - latest_price) / latest_price if latest_price > 0 else 0
            ),
            "model_info": {
        ...
        try:
            ...
            if HAS_STATSMODELS and len(df) >= 30:
                ...
                for i in range(days):
                    ...
                    pred_date = (
                        pd.Timestamp.now() + pd.Timedelta(days=i + 1)
                    ).strftime("%Y-%m-%d")
                    predictions.append(
                        {
                            "date": pred_date,
    ...
    def _lstm_predict(self, df: pd.DataFrame, days: int) -> Dict:
        ...
        try:
            ...
            trend_strength = float(
                (short_ema.iloc[-1] - long_ema.iloc[-1]) / long_ema.iloc[-1]
            )
            ...
            for i in range(days):
                ...
                pred_price = latest_price * (
                    1 + momentum * decay + trend_strength * decay * 0.5
                )
    ...
    def _ensemble_predict(
        self, predictions: List[Dict], latest_price: Optional[float] = None
    ) -> Dict:
        """集成預測。
        ...
        try:
            ...
            return {
                ...
                "model_agreement": np.mean(
                    [pred["model_agreement"] for pred in ensemble_predictions]
                ),
            }
    ...
    def _evaluate_model_performance(self, split_data: Dict) -> Dict:
        ...
        try:
            ...
            if len(test_data) > 1:
                ...
                baseline_mape = (
                    float(np.mean(mape_vals) * 100) if len(mape_vals) > 0 else 0
                )
    ...
    def _generate_prediction_summary(self, prediction_result: Dict) -> Dict:
        ...
        try:
            ...
            if "ensemble" in predictions:
                ...
                # 計算波動性
                pred_prices = [
                    p["predicted_price"] for p in ensemble_pred.get("predictions", [])
                ]
                if pred_prices:
