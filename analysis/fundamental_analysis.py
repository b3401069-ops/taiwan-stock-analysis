...
class FundamentalAnalysis:
    ...
    async def analyze(self, stock_id: str) -> Dict:
        ...
        try:
            ...
            # 計算財務比率
            analysis_result["financial_analysis"] = self._calculate_financial_ratios(
                financial_data, realtime_data
            )

            # 分析獲利能力
            analysis_result["profitability"] = self._analyze_profitability(
                financial_data
            )
            ...
            # 計算估值指標
            analysis_result["valuation_metrics"] = self._calculate_valuation_metrics(
                financial_data, realtime_data
            )
    ...
    def _calculate_financial_ratios(
        self, financial_data: Dict, realtime_data: Dict
    ) -> Dict:
        """計算財務比率"""
        try:
            ratios = {}
            ...
            if financial_data.get("income_statement"):
                ...
                if "Gross Profit" in income_data and "Total Revenue" in income_data:
                    ratios["gross_margin"] = (
                        income_data["Gross Profit"] / income_data["Total Revenue"]
                    )

                # 計算營業利益率
                if "Operating Income" in income_data and "Total Revenue" in income_data:
                    ratios["operating_margin"] = (
                        income_data["Operating Income"] / income_data["Total Revenue"]
                    )

                # 計算淨利率
                if "Net Income" in income_data and "Total Revenue" in income_data:
                    ratios["net_margin"] = (
                        income_data["Net Income"] / income_data["Total Revenue"]
                    )
            ...
            if financial_data.get("balance_sheet"):
                ...
                if "Total Debt" in balance_data and "Total Assets" in balance_data:
                    ratios["debt_ratio"] = (
                        balance_data["Total Debt"] / balance_data["Total Assets"]
                    )

                # 計算流動比率
                if (
                    "Current Assets" in balance_data
                    and "Current Liabilities" in balance_data
                ):
                    ratios["current_ratio"] = (
                        balance_data["Current Assets"]
                        / balance_data["Current Liabilities"]
                    )

                # 計算速動比率
                if (
                    "Current Assets" in balance_data
                    and "Inventory" in balance_data
                    and "Current Liabilities" in balance_data
                ):
                    ratios["quick_ratio"] = (
                        balance_data["Current Assets"] - balance_data["Inventory"]
                    ) / balance_data["Current Liabilities"]
            ...
            if financial_data.get("cash_flow"):
                ...
                # 計算營業現金流對淨利比率
                if (
                    "Operating Cash Flow" in cashflow_data
                    and "Net Income" in cashflow_data
                ):
                    ratios["ocf_to_net_income"] = (
                        cashflow_data["Operating Cash Flow"]
                        / cashflow_data["Net Income"]
                    )
    ...
    def _analyze_profitability(self, financial_data: Dict) -> Dict:
        """分析獲利能力"""
        try:
            profitability = {"metrics": {}, "trend": "stable", "rating": "average"}
            ...
            if financial_data.get("income_statement"):
                ...
                for year in years:
                    ...
                    # 計算營業利益率
                    if (
                        "Operating Income" in income_data
                        and "Total Revenue" in income_data
                    ):
                ...
                if gross_margins:
                    profitability["metrics"]["gross_margin"] = {
                        ...
                        "trend": (
                            ...
                            and gross_margins[-1] > gross_margins[-2]
                            else "down"
                        ),
                    }
                ...
                if operating_margins:
                    profitability["metrics"]["operating_margin"] = {
                        ...
                        "trend": (
                            ...
                            and operating_margins[-1] > operating_margins[-2]
                            else "down"
                        ),
                    }
                ...
                if net_margins:
                    profitability["metrics"]["net_margin"] = {
                        ...
                        "trend": (
                            "up"
                            if len(net_margins) > 1
                            and net_margins[-1] > net_margins[-2]
                            else "down"
                        ),
                    }
    ...
    def _analyze_growth(self, financial_data: Dict) -> Dict:
        """分析成長性"""
        try:
            growth = {"metrics": {}, "trend": "stable", "rating": "average"}
            ...
            if financial_data.get("income_statement"):
                ...
                for i in range(1, len(years)):
                    ...
                    # 計算營收成長率
                    if (
                        "Total Revenue" in current_data
                        and "Total Revenue" in previous_data
                    ):
                        if previous_data["Total Revenue"] != 0:
                            growth_rate = (
                                current_data["Total Revenue"]
                                - previous_data["Total Revenue"]
                            ) / previous_data["Total Revenue"]
                    ...
                    if "Net Income" in current_data and "Net Income" in previous_data:
                        if previous_data["Net Income"] != 0:
                            growth_rate = (
                                current_data["Net Income"] - previous_data["Net Income"]
                            ) / previous_data["Net Income"]
                ...
                if revenue_growth:
                    growth["metrics"]["revenue_growth"] = {
                        ...
                        "trend": (
                            ...
                            and revenue_growth[-1] > revenue_growth[-2]
                            else "down"
                        ),
                    }
                ...
                if net_income_growth:
                    growth["metrics"]["net_income_growth"] = {
                        ...
                        "trend": (
                            "up"
                            if len(net_income_growth) > 1
                            and net_income_growth[-1] > net_income_growth[-2]
                            else "down"
                        ),
                    }
    ...
    def _analyze_stability(self, financial_data: Dict) -> Dict:
        """分析穩定性"""
        try:
            stability = {"metrics": {}, "trend": "stable", "rating": "average"}
            ...
            if financial_data.get("income_statement"):
                ...
                if revenue_volatility:
                    stability["metrics"]["revenue_volatility"] = {
                        ...
                        "trend": (
                            "up"
                            if len(revenue_volatility) > 1
                            and revenue_volatility[-1] > revenue_volatility[-2]
                            else "down"
                        ),
                    }
                ...
                if net_income_volatility:
                    stability["metrics"]["net_income_volatility"] = {
                        ...
                        "trend": (
                            "up"
                            if len(net_income_volatility) > 1
                            and net_income_volatility[-1] > net_income_volatility[-2]
                            else "down"
                        ),
                    }
    ...
    def _calculate_valuation_metrics(
        ...
        try:
            ...
            if financial_data.get("balance_sheet"):
                ...
                if (
                    "Stockholders Equity" in balance_data
                    and balance_data["Stockholders Equity"] != 0
                ):
                    # 假設股數為1億股（實際應該從資料獲取）
                    shares_outstanding = 100000000
                    book_value_per_share = (
                        balance_data["Stockholders Equity"] / shares_outstanding
                    )
            ...
            if valuation["pe_ratio"] > 0:
                ...
                valuation["peg_ratio"] = valuation["pe_ratio"] / (
                    assumed_growth_rate * 100
                )
