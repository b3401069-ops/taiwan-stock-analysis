...
class ResearchReportGenerator:
    ...
    def generate_portfolio_report(self, report_type: str = "weekly") -> Dict:
        ...
        try:
            ...
            # 產生整體報告
            report = self._compile_report(stock_reports, portfolio_data["data"], report_type)
    ...
    def generate_single_stock_report(self, stock_id: str) -> Dict:
        ...
        try:
            ...
            report = {
                ...
                # 估值指標
                "valuation": valuation.get("data", {}) if valuation.get("success") else {},
    ...
    def _analyze_stock_for_report(self, position: Dict) -> Dict:
        ...
        try:
            ...
            distance_to_target = ((target_price - current_price) / current_price * 100) if target_price > 0 else 0
            distance_to_stop = ((stop_loss - current_price) / current_price * 100) if stop_loss > 0 else 0
            
            # 生成建議
            suggestion = self._generate_suggestion(
                current_price, entry_price, target_price, stop_loss,
                trend, risk, valuation_data, profit_loss_pct
            )
    ...
    def _generate_suggestion(self, current_price, entry_price, target_price, stop_loss,
                            trend, risk, valuation, profit_loss_pct) -> Dict:
        """生成建議"""
        try:
            # 趨勢評分
            ...
            # 估值評級
            valuation_rating = valuation.get("valuation_rating", {}).get("label", "中性")
    ...
    def _compile_report(self, stock_reports: List[Dict], portfolio_data: Dict, report_type: str) -> Dict:
        """組合完整報告"""
        try:
            # 統計數據
            total_positions = len(stock_reports)
            profitable_positions = len([r for r in stock_reports if r.get("profit_loss", 0) > 0])
            losing_positions = total_positions - profitable_positions
            
            total_profit_loss = sum(r.get("profit_loss", 0) for r in stock_reports)
            avg_return = np.mean([r.get("profit_loss_pct", 0) for r in stock_reports]) if stock_reports else 0
            
            # 找出最佳和最差
            best_stock = max(stock_reports, key=lambda x: x.get("profit_loss_pct", 0)) if stock_reports else None
            worst_stock = min(stock_reports, key=lambda x: x.get("profit_loss_pct", 0)) if stock_reports else None
            
            # 生成文字報告
            report_text = self._generate_report_text(
                stock_reports, portfolio_data, report_type,
                total_positions, profitable_positions, losing_positions,
                total_profit_loss, avg_return, best_stock, worst_stock
            )
    ...
    def _generate_report_text(self, stock_reports, portfolio_data, report_type,
                             total_positions, profitable_positions, losing_positions,
                             total_profit_loss, avg_return, best_stock, worst_stock) -> str:
        """生成文字報告"""
        try:
            today = datetime.now()
...
"""
    ...
    def _generate_overall_suggestion(self, stock_reports: List[Dict]) -> str:
        ...
        try:
            ...
            # 獲利了結
            take_profit_count = suggestions.get("take_profit", 0) + suggestions.get("consider_sell", 0)
            if take_profit_count > 0:
                advice.append(f"🟢 **{take_profit_count} 檔股票達到目標**，可考慮獲利了結")
