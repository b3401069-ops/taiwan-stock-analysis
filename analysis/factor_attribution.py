...
class FactorAttribution:
    ...
    def analyze(
        ...
        try:
            ...
            logger.info(
                f"五因子歸因分析完成，R² = {regression_result['r_squared']:.3f}"
            )
    ...
    def _run_regression(
        self, X: pd.DataFrame, y: pd.Series, risk_free_rate: float
    ) -> Dict:
        """執行回歸分析"""
        try:
            # 計算超額報酬（減去無風險利率）
    ...
    def _calculate_factor_exposures(
        self, regression_result: Dict, factor_returns: Dict[str, pd.Series]
    ) -> List[FactorExposure]:
        """計算因子暴露和貢獻"""
        try:
            ...
            for i, factor_name in enumerate(factor_names):
                if i < len(coefficients):
                    ...
                    contribution = (
                        exposure * factor_returns[factor_name].mean() * 252
                    )  # 年化貢獻
    ...
    def _calculate_portfolio_returns(
        self, all_returns: Dict[str, pd.Series], weights: List[float]
    ) -> pd.Series:
        """計算投資組合報酬率"""
        try:
            # 對齊時間序列
