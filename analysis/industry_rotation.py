...
class IndustryRotationAnalyzer:
    ...
    def find_rotation_opportunities(self, period: str = "6mo") -> List[RotationOpportunity]:
        """
        尋找輪動機會
        ...
        try:
            ...
            if strongest.strength_score > 0.3 and weakest.strength_score < -0.3:
                ...
                opportunity = RotationOpportunity(
                    ...
                    signal_strength=min(abs(strongest.strength_score - weakest.strength_score), 1.0),
                    expected_return=(strongest.strength_score - weakest.strength_score) * 10,  # 簡化預期報酬
                    risk_level="中",
            ...
            for i in range(len(industry_strengths) - 1):
                ...
                if current.strength_score - next_industry.strength_score > 0.4:
                    opportunity = RotationOpportunity(
                        ...
                        signal_strength=min(current.strength_score - next_industry.strength_score, 1.0),
                        expected_return=(current.strength_score - next_industry.strength_score) * 8,
                        risk_level="低",
    ...
    def _analyze_industry_strength(
        ...
        try:
            ...
            # 計算動量指標
            momentum_20d = industry_returns.pct_change(periods=20).iloc[-1] if len(industry_returns) >= 20 else 0
            momentum_60d = industry_returns.pct_change(periods=60).iloc[-1] if len(industry_returns) >= 60 else 0
            
            # 計算相對強度（與加權指數比較）
            relative_strength = self._calculate_relative_strength(industry_returns, period)
            ...
            strength_score = self._calculate_strength_score(
                momentum_20d, momentum_60d, relative_strength, 
                turnover_rate, institutional_flow
            )
    ...
    def _calculate_industry_returns(self, stocks: List[str], period: str) -> Optional[pd.Series]:
        """計算產業指數報酬率"""
        try:
            # 取得所有股票的報酬率
            all_returns = []
    ...
    def _calculate_relative_strength(self, industry_returns: pd.Series, period: str) -> float:
        """計算相對強度（與加權指數比較）"""
        try:
            # 取得加權指數報酬率
            ...
            # 計算相對強度
            relative_strength = (industry_aligned.iloc[-1] / taiex_aligned.iloc[-1] - 1) * 100
    ...
    def _calculate_strength_score(
        ...
        try:
            ...
            # 綜合分數
            total_score = momentum_score + rs_score + turnover_score + institutional_score
        ...
        try:
            ...
            return {
                ...
                "data": {
                    ...
                    "summary": {
                        "strongest_industry": ranking[0]["industry"] if ranking else "N/A",
                        "weakest_industry": ranking[-1]["industry"] if ranking else "N/A",
