    ...
    def get_realtime_quote(self, stock_id: str) -> Dict:
        ...
        try:
            ...
            result = {
                ...
                "orderbook": {
                    "bid": [
                        {"price": float(quote.get(f"bid_price_{i}", 0)), "volume": int(quote.get(f"bid_volume_{i}", 0))}
                        for i in range(1, 6)
                    ],
                    "ask": [
                        {"price": float(quote.get(f"ask_price_{i}", 0)), "volume": int(quote.get(f"ask_volume_{i}", 0))}
                        for i in range(1, 6)
                    ]
                },
...
async def get_financial(
    stock_id: str,
    report_type: str = Query("ratios", description="報表類型 (income, balance, cashflow, ratios)")
):
    """取得財報數據"""
