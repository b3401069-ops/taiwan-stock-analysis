    ...
    if args.watchlist_action == "add":
        ...
        if watchlist:
            for i, stock in enumerate(watchlist, 1):
                print(f"{i:2d}. {stock.get('stock_id', 'N/A')} - {stock.get('name', 'N/A')}")
...
def report_command(args):
    ...
    if args.report_action == "daily":
        ...
        if result and result.get("success"):
            ...
            if args.notify:
                print("\n📱 發送通知...")
                call_api("/notification/discord/report", method="POST", params={"report_type": "每日報告"})
                call_api("/notification/line/report", method="POST", params={"report_type": "每日報告"})
                print("✅ 通知已發送")
        else:
            print("❌ 無法產生每日報告")
        ...
        if industry_result and industry_result.get("success"):
            for item in industry_result["data"][:3]:
                print(f"  • {item['industry']}: {item['strength_score']:.4f} ({item['signal']})")
        ...
        if args.notify:
            print("\n📱 發送通知...")
            call_api("/notification/discord/report", method="POST", params={"report_type": "每週報告"})
            call_api("/notification/line/report", method="POST", params={"report_type": "每週報告"})
            print("✅ 通知已發送")
        ...
        if args.notify:
            print("\n📱 發送通知...")
            call_api("/notification/discord/report", method="POST", params={"report_type": "每月報告"})
            call_api("/notification/line/report", method="POST", params={"report_type": "每月報告"})
            print("✅ 通知已發送")
        ...
        if stocks:
            ...
            if args.export:
                ...
                with open(args.export, 'w', newline='', encoding='utf-8-sig') as f:
                    writer = csv.writer(f)
                    writer.writerow(['排名', '股票代碼', '股票名稱', '綜合分數', '目前價格'])
            ...
            if args.notify:
                print("\n📱 發送通知...")
                call_api("/notification/discord/report", method="POST", params={"report_type": "選股掃描結果"})
                call_api("/notification/line/report", method="POST", params={"report_type": "選股掃描結果"})
                print("✅ 通知已發送")
        else:
            print("❌ 沒有篩選結果")
    else:
...
def morning_routine_command(args):
    ...
    if args.notify:
        ...
        # 發送 Discord 通知
        call_api("/notification/discord/report", method="POST", params={"report_type": "每日早晨報告"})
        
        # 發送 Line 通知
        call_api("/notification/line/report", method="POST", params={"report_type": "每日早晨報告"})
...
def notify_command(args):
    ...
    if args.message:
        ...
        # 發送 Discord 通知
        call_api("/notification/discord", method="POST", params={"content": args.message})
...
def main():
    ...
    # watchlist remove
    watchlist_remove_parser = watchlist_subparsers.add_parser("remove", help="移除關注股票")
    ...
    report_daily_parser.add_argument("--top-n", type=int, default=10, help="返回前 N 名")
    ...
    # 每日早晨例行流程命令
    morning_parser = subparsers.add_parser("morning-routine", help="每日早晨例行流程命令")
